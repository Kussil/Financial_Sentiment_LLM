...
index = pc.Index("fastfullvectors")
...
def ask_vector_query(query, top_results, ticker, date, num_days_back, df_chunk, up_down):
    try:
...
df_sentiment = load_parquet('03_Sentiment_Analysis/Gemini/Prompt2/Prompt2_Sentiment_Analysis_Results.parquet')

# Load Vector Full Article References
full_files = [
    '05_Create_Vector_DB/Gemini/Article_Full_References_pt1.parquet',
    '05_Create_Vector_DB/Gemini/Article_Full_References_pt2.parquet',
    '05_Create_Vector_DB/Gemini/Article_Full_References_pt3.parquet'
]
df_full = pd.concat([load_parquet(file) for file in full_files], ignore_index=True)
...
article_files = [
    '02_Cleaned_Data/Investment_Research_Part1.parquet',
    '02_Cleaned_Data/Investment_Research_Part2.parquet',
    '02_Cleaned_Data/ProQuest_Articles.parquet',
    '02_Cleaned_Data/Earnings_Presentations.parquet',
    '02_Cleaned_Data/Earnings_QA.parquet',
    '02_Cleaned_Data/SEC_Filings.parquet'
]
articles_dfs = [load_parquet(file)[columns_to_load] for file in article_files]
...
try:
    if selected_date != None and st.session_state.stock_change != None:
        st.markdown("<h2 style='text-align: center;'>" + \
                    f'What potential factors caused {ticker} stock price to {up_down} on {str(selected_date)}?' + \
        ...
        if st.button('Generate Response from Google Gemini') and query:
            response_vector, article_ids = ask_vector_query(query, selected_chunk_count, ticker, selected_date, num_days_back, df_full, None)
            st.session_state.response_vector = response_vector.replace('$','\$') if response_vector else None
...
if st.session_state.response_vector:
    ...
    if st.button('Ask Question') and ask_query:
        ask_response, ask_article_ids = ask_vector_query(ask_query, selected_ask_chunk_count, ticker, ask_selected_date, num_days_back, df_full, None)
